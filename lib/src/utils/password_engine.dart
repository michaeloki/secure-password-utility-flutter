...
Future<bool> weakPasswordChecker(String rawPassword, int passwordLength) async {
  ...
  try {
    if (rawPassword.isEmpty || passwordLength == 0) {
      print(
          'SecurePasswordUtility::: empty password and/or zero length is not allowed');
      return !status;
    } else {
      try {
        if (!(passwordLength == rawPassword.length && passwordLength >= 15)) {
          print(
...
Future<String> strongPasswordGenerator(int passwordLength) async {
  ...
  scrambledPassword = upperCaseGenerator +
      lowerCaseGenerator +
      numericGenerator +
      characterGenerator;
  ...
  if (remainder != 0) {
    ...
    return completePasswordGeneration(
        passwordLength, autoGeneratedPassword, scrambledPassword);
  } else {
    return completePasswordGeneration(
        passwordLength, autoGeneratedPassword, scrambledPassword);
  }
...
String completePasswordGeneration(
    int passwordLength, autoGeneratedPassword, scrambledPassword) {
  for (var b = 0; b < passwordLength; b++) {
    autoGeneratedPassword +=
        scrambledPassword[Random().nextInt(scrambledPassword.length)];
  }
...
String productKeyGenerator(int productKeyLength) {
  ...
  if (productKeyLength <= 15) {
    print(
        'SecurePasswordUtility::: The length of your product key must be greater than 15 and should be a multiple of 4 or 5 e.g. 16 or 20');
    return '';
  }
